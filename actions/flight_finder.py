...
#   5. Parse with Gemini â†’ structured flight data
#   6. Speak top results
#   7. Optionally save to Notepad or keep browser open
#
# Cross-platform: Windows, macOS, Linux
# No API key required â€” Google Flights is free to access
...
def _parse_date(raw: str) -> str:
    ...
    month_map = {
        ...
        "january": 1,  "ocak": 1,      "ÅŸubat": 2,      "mart": 3,
        "nisan": 4,    "mayÄ±s": 5,     "haziran": 6,    "temmuz": 7,
        "aÄŸustos": 8,  "eylÃ¼l": 9,     "ekim": 10,      "kasÄ±m": 11,
        "aralÄ±k": 12,
    }
...
def _parse_flights_with_gemini(
    ...
    try:
        ...
        print(f"[FlightFinder] âš ï¸ Parse failed: {e}")
        return []
...
def _format_spoken(
    ...
    """Formats flights for spoken output â€” concise and natural."""
    if not flights:
        return (
...
def _format_notepad(
    ...
    """Formats flights for Notepad â€” detailed and readable."""
...
def flight_finder(
    ...
    """
    Flight Finder â€” searches Google Flights and speaks results.

    Parameters:
        origin       (str, required) â€” departure city or airport (e.g. "Istanbul", "IST")
        destination  (str, required) â€” arrival city or airport (e.g. "London", "LHR")
        date         (str, required) â€” departure date (any format: "15 Mart", "March 15", "2025-03-15")
        return_date  (str, optional) â€” return date for round trips
        passengers   (int, optional) â€” number of passengers (default: 1)
        cabin        (str, optional) â€” economy | premium | business | first (default: economy)
        save         (bool, optional) â€” save results to Notepad (default: False)
        show_browser (bool, optional) â€” keep browser open after search (default: True)
    ...
    print(f"[FlightFinder] â–¶ï¸ {origin} â†’ {destination} | {date} | {cabin} | {passengers} pax")
    ...
    try:
        ...
        print(f"[FlightFinder] âŒ Error: {e}")
