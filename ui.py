...
SYSTEM_NAME = "JARVIS"
MODEL_BADGE = "© Alinshan - J.A.R.V.I.S"

C_BG     = "#020405"
C_PRI    = "#00f2ff"  # Arc Reactor Cyan
C_MID    = "#0088cc"
C_DIM    = "#004466"
C_DIMMER = "#001a22"
C_ACC    = "#e61919"  # Stark Red
C_ACC2   = "#ffcc00"  # Stark Gold
C_TEXT   = "#dbf9ff"
C_PANEL  = "#030a0e"
C_GREEN  = "#00ffcc"
C_RED    = "#ff1122"
...
class JarvisUI:
    def __init__(self, face_path, size=None):
        ...
        # Hex grid state
        self.hex_offset = 0.0
        self.readout_vals = [random.uniform(90, 99) for _ in range(4)]
        ...
        self._api_key_ready = is_configured()
    ...
    def set_state(self, state: str):
        ...
        state: LISTENING | SPEAKING | THINKING | MUTED | ONLINE | PROCESSING | ASLEEP
        """
        self._jarvis_state = state
        
        # Wake up UI if it was hidden
        if state not in ["ASLEEP", "MUTED", "OFFLINE"]:
            self.show_window()
        ...
        if state == "ASLEEP":
            ...
            self.status_text = "READY // PASSIVE"
            self.speaking    = False
        else:
            self.status_text = "ONLINE"
            self.speaking    = False
    ...
    def _animate(self):
        ...
        if now - self.last_t > (0.14 if self.speaking else 0.55):
            ...
            elif self._jarvis_state == "ASLEEP":
                self.target_scale = random.uniform(0.999, 1.002)
                self.target_halo  = random.uniform(25, 40)
            else:
        ...
        self.hex_offset = (self.hex_offset + 0.35) % 88
        for i in range(4):
            self.readout_vals[i] = max(80, min(100, self.readout_vals[i] + random.uniform(-0.5, 0.5)))
    ...
    def _draw(self):
        ...
        for x in range(-hex_s, W + hex_s, hex_s):
            for y in range(-hex_s, H + hex_s, int(hex_s * 1.5)):
                yo = (hex_s * 0.75) if (x // hex_s) % 2 else 0
                xx, yy = x + off, y + yo + off*0.5
                # Sadece noktalar yerine küçük altıgen köşeleri
                c.create_oval(xx-1, yy-1, xx+1, yy+1, fill=C_DIMMER, outline="")

        # Tactical Side Readouts
        RX = 30
        RY = 100
        for i, (label, val) in enumerate([("CORE", self.readout_vals[0]), ("SYNC", self.readout_vals[1]), ("POWR", self.readout_vals[2]), ("TEMP", self.readout_vals[3])]):
            ly = RY + i * 50
            c.create_text(RX, ly, text=label, fill=C_DIM, font=("Courier", 8, "bold"), anchor="w")
            c.create_rectangle(RX, ly+10, RX+80, ly+14, outline=C_DIMMER, fill="#01080b")
            c.create_rectangle(RX, ly+10, RX + int(80*val/100), ly+14, fill=C_PRI if val > 90 else C_ACC2, outline="")
        ...
        for r in range(int(FW * 0.54), int(FW * 0.28), -22):
            ...
            if self.muted:
                ...
                c.create_oval(FCX-r, FCY-r, FCX+r, FCY+r,
                              outline=f"#{gh}1100", width=2)
            else:
                gh = f"{ga:02x}"
                c.create_oval(FCX-r, FCY-r, FCX+r, FCY+r,
                              outline=f"#00{gh}{gh}", width=2)
        ...
        for pr in self.pulse_r:
            ...
            if self.muted:
                c.create_oval(FCX-r, FCY-r, FCX+r, FCY+r,
                              outline=self._ac(230, 25, 25, pa // 3), width=2)
            else:
                c.create_oval(FCX-r, FCY-r, FCX+r, FCY+r,
                              outline=self._ac(0, 242, 255, pa), width=2)
        ...
        scan_col = self._ac(230, 25, 25, scan_a) if self.muted else self._ac(0, 242, 255, scan_a)
        ...
        c.create_arc(FCX-sr, FCY-sr, FCX+sr, FCY+sr,
                     ...
                     outline=self._ac(255, 215, 0, scan_a // 2), width=2, style="arc")
        ...
        ch_r = int(FW * 0.50)
        gap  = int(FW * 0.15)
        ch_a = self._ac(0, 242, 255, int(self.halo_a * 0.55))
        for x1, y1, x2, y2 in [
        ...
        if self._has_face:
            ...
            # Stark Arc Reactor redesign
            orb_r = int(FW * 0.27 * self.scale)
            # Power cells (mechanical look)
            for i in range(10):
                angle = math.radians(i * 36 + self.rings_spin[0])
                ox = FCX + math.cos(angle) * (orb_r + 5)
                oy = FCY + math.sin(angle) * (orb_r + 5)
                c.create_rectangle(ox-4, oy-4, ox+4, oy+4, fill=C_PRI if not self.muted else C_ACC, outline=C_DIM)
            
            # Glowing core
            orb_color = (255, 30, 80) if self.muted else (0, 242, 255)
            ...
            c.create_text(FCX, FCY, text=SYSTEM_NAME,
                          fill=self._ac(219, 249, 255, min(255, int(self.halo_a * 2.2))),
                          font=("Courier", 16, "bold"))
            
            # Inner details
            c.create_oval(FCX-orb_r-10, FCY-orb_r-10, FCX+orb_r+10, FCY+orb_r+10, outline=C_DIM, width=1)
        ...
        c.create_text(W // 2, 44, text="STARK INDUSTRIES PROTOCOL // ACTIVE",
                      fill=C_DIM, font=("Courier", 8, "bold"))
        c.create_text(16, 31, text="JARVIS_v1.0",
                      fill=C_DIM, font=("Courier", 9, "bold"), anchor="w")
        c.create_text(W - 16, 31, text=time.strftime("%H:%M:%S"),
                      fill=C_ACC2, font=("Courier", 14, "bold"), anchor="e")
        ...
        c.create_text(W // 2, H - 14, fill=C_DIM, font=("Courier", 8),
                      text="Developer - Alinshan  ·  CLASSIFIED  ·  JARVIS")
    ...
    def _show_setup_ui(self):
        ...
        tk.Label(self.setup_frame, text="◈  INITIALISATION REQUIRED",
                 fg=C_PRI, bg="#01080b", font=("Courier", 13, "bold")).pack(pady=(18, 4))
        tk.Label(self.setup_frame,
                 text="Enter your Gemini API key to boot JARVIS.",
                 fg=C_DIM, bg="#01080b", font=("Courier", 9)).pack(pady=(0, 10))

        tk.Label(self.setup_frame, text="GEMINI API KEY",
                 fg=C_DIM, bg="#01080b", font=("Courier", 9)).pack(pady=(8, 2))
